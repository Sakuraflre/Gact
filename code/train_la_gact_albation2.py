...
from test_LA import test_calculate_metric
from networks.vnet_sdf import VNet
from dataloaders import utils
from utils import ramps, losses, metrics
from dataloaders.process import LAHeart, RandomCrop, CenterCrop, RandomRotFlip, ToTensor, TwoStreamBatchSampler
...
parser.add_argument('--exp', type=str,
                    default='LA/GACT_with_Ablation_2', help='model_name')
...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_sdf = (mse_loss(outputs_tanh[:labeled_bs, 0, ...], gt_dis) + mse_loss(
                outputs_tanh2[:labeled_bs, 0, ...], gt_dis)) * 0.5
            # print(outputs_soft)
          ...
          #  print( torch.norm(outputs_tanh[:labeled_bs, 0, ...],p=2 ))
            ...
            # writer.add_scalar('loss/loss', loss, iter_num)
            logging.info(
                'iteration %d : loss : %f, loss_sdf: %f, loss_seg: %f' %
                (iter_num, loss.item(), loss_sdf.item(), loss_seg ))
