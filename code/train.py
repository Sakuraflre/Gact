...
parser.add_argument('--dataset', type=str,
                    default='BRATS', help='Name of Experiment')
parser.add_argument('--root_path', type=str,
                    default='../data', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='GACT_with_consis_weight', help='model_name')
...
parser.add_argument('--trainnum', type=int,  default=240, help='random seed')
parser.add_argument('--testnum', type=int,  default=60, help='random seed')

parser.add_argument('--labelnum', type=int,  default=24, help='random seed')
...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs_tanh, outputs, outputs_tanh2, outputs2 = model(volume_batch)
            outputs_soft = torch.sigmoid(outputs)
            outputs_soft2 = torch.sigmoid(outputs)
            ...
            p = 2.0
            w1 = float(np.exp(-1 * p * torch.abs((outputs_tanh[:labeled_bs, 0, ...]).sum()/outputs_tanh.nelement()).item()))
            w2 = float(np.exp(-1 * p * torch.abs((outputs_tanh2[:labeled_bs, 0, ...]).sum()/outputs_tanh2.nelement()).item()))
            # print(outputs_soft)
            # print(torch.norm(outputs_soft[:labeled_bs, 0, ...], p=2))
          #  print( torch.norm(outputs_tanh[:labeled_bs, 0, ...],p=2 ))
            #print(w1)
            loss_sdf = (mse_loss(outputs_tanh[:labeled_bs, 0, ...], gt_dis) + mse_loss(outputs_tanh2[:labeled_bs, 0, ...], gt_dis)) * 0.5


            loss_seg_ce = ce_loss(outputs[:labeled_bs, 0, ...], label_batch[:labeled_bs].float())
            loss_seg_dice = losses.dice_loss(outputs_soft[:labeled_bs, 0, :, :, :], label_batch[:labeled_bs] == 1)
            loss_seg_ce2 = ce_loss(outputs2[:labeled_bs, 0, ...], label_batch[:labeled_bs].float())
            loss_seg_dice2 = losses.dice_loss(outputs_soft2[:labeled_bs, 0, :, :, :], label_batch[:labeled_bs] == 1)

            loss_seg = (loss_seg_ce + loss_seg_dice + loss_seg_ce2 + loss_seg_dice2) * 0.5

            dis_to_mask = torch.sigmoid(-1500*outputs_tanh)
            dis_to_mask2 = torch.sigmoid(-1500 * outputs_tanh2)

            consistency_loss = w1*torch.mean((outputs_soft - dis_to_mask2) ** 2) + w2*torch.mean((outputs_soft2 - dis_to_mask) ** 2)
           # print(consistency_loss.device)
            ...
            #print(metric)
            logging.info(
                'iteration %d : loss : %f, loss_consist: %f, loss_sdf: %f, loss_seg: %f, consistency_weight: %f' %
                (iter_num, loss.item(), consistency_loss.item(), loss_sdf.item(), loss_seg, consistency_weight))
            ...
            if iter_num % 50 == 0:
                ...
                image = outputs_soft2[0, 0:1, :, :, 20:61:10].permute(
                    3, 0, 1, 2).repeat(1, 3, 1, 1)
                grid_image = make_grid(image, 5, normalize=False)
                writer.add_image('train/Predicted_label2', grid_image, iter_num)
                ...
                image = dis_to_mask2[0, 0:1, :, :, 20:61:10].permute(
                    3, 0, 1, 2).repeat(1, 3, 1, 1)
                grid_image = make_grid(image, 5, normalize=False)
                writer.add_image('train/Dis2Mask2', grid_image, iter_num)
                ...
                image = outputs_tanh2[0, 0:1, :, :, 20:61:10].permute(
                    3, 0, 1, 2).repeat(1, 3, 1, 1)
                grid_image = make_grid(image, 5, normalize=False)
                writer.add_image('train/DistMap2', grid_image, iter_num)
            ...
            if iter_num % 2500 == 0:
                ...
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr_
            if iter_num % 100 == 0 and iter_num >= 3500:
            #if iter_num % 1 == 0 :
                ...
                metric = test_calculate_metric_memory(model, train_data_path, save_test_path, args.testnum)
            #metric = test_calculate_metric(save_mode_path,   train_data_path  , save_test_path, args.testnum )  # 6000
                logging.info(
